...
void loop() {
  ...
  if (cmd.params[0] == 2) {
    int nicla_flag = (int)senses[niclaOffset + 0]; //FLAG FROM ARDUINO -- 1 if detected or 0 if non-detected
    if (nicla_flag != 0) {// checks if new yaw occurs -- blob detected
        float _yaw = senses[5];  
    ...
    behave.params[2] = cmd.params[2] + z_estimator; // fz (meters)
